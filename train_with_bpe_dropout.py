...
def get_args():
    ...
    # __BPE dropout: add an argument to enable BPE dropout
    parser.add_argument('--bpe-dropout', default=None, type=str, help='a preprocessing script to train model with BPE dropout')

    # Add model arguments
    parser.add_argument('--arch', default='lstm', choices=ARCH_MODEL_REGISTRY.keys(), help='model architecture')

    # Add optimization arguments
    parser.add_argument('--max-epoch', default=150, type=int, help='force stop training at specified epoch')
...
def main(args):
    ...
    for epoch in range(last_epoch + 1, args.max_epoch):
        # __BPE dropout: before every epoch call an external bash script that resegments the training data, then reload the data
        if args.bpe_dropout:
            logging.info("Executing external preprocessing script...")
            if args.train_on_tiny:
                proc = subprocess.Popen([args.bpe_dropout, "tiny_train"])
            else:
                proc = subprocess.Popen([args.bpe_dropout, "train"])
            proc.wait()
            logging.info("Training data is newly re-segmented!")
