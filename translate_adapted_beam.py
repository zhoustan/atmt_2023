...
from seq2seq.adapted_beam import AdaptedBeamSearch
from seq2seq.beam import BeamSearchNode
...
def get_args():
    ...
    parser.add_argument('--gamma', default=0.1, type=float, help='gamma for penalizing adapted beam search')
...
def main(args):
    ...
    for i, sample in enumerate(progress_bar):
        ...
        searches = [AdaptedBeamSearch(args.beam_size, args.max_len - 1, tgt_dict.unk_idx, args.gamma) for i in range(batch_size)]
        ...
        for i in range(batch_size):
            for j in range(args.beam_size):
                ...
                searches[i].add(-node.eval(args.alpha), node, sibling_rank)
            ...
            for i in range(log_probs.shape[0]):
                for j in range(args.beam_size):
                    ...
                    if next_word[-1] == tgt_dict.eos_idx:
                        ...
                        search.add_final(-node.eval(args.alpha), node, sibling_rank)
                    ...
                    # Add the node to current nodes for next iteration
                        ...
                        search.add(-node.eval(args.alpha), node, sibling_rank)
